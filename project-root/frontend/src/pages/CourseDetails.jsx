...
export default function CourseDetail({ courseId }) {
    const { setSelectedComponent, user, getCourse, coursesLoading, coursesError, refreshCourses, backgroundColor, textColor, BACKEND_URL } = useContext(GlobalContext);
    ...
    return (
        <div className={styles.courseDetailContainer} style={{ backgroundColor: backgroundColor }}>
            <div className={styles.courseHeader}>
                <button className={styles.backButton} onClick={handleBackClick} style={{ color: textColor }}>
                    <FaArrowLeft /> Back to Courses
                </button>
                <h1 className={styles.courseTitle} style={{ color: textColor }}>{course.name}</h1>
                <p className={styles.courseDescription} style={{ color: textColor}}>{course.description}</p>
            </div>
            <TabNav
                tabs={courseTabs}
                activeTab={activeTab}
